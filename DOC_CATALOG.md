...
| `Docs\roadmap\SDK55_Upgrade_And_Analytics_Plan.md` | Execution runbook to upgrade Expo SDK 54→55 and turn on Firebase Analytics (roadmap #9). | Plan the SDK upgrade + analytics activation, or follow the phased steps/risks. | Verified current state (SDK 54, RN 0.81, Firebase not installed, `GoogleService-Info.plist` present / `google-services.json` missing, wrapper wired); Phase 1 SDK 54→55 upgrade (`expo install --fix`, doctor, jest/Maestro/regression); Phase 2 `@react-native-firebase` + iOS static frameworks + config files; Phase 3 privacy-declaration updates + build/submit; risks; definition of done. |
