...
| `Docs\roadmap\Stock_Image_Implementation_Plan.md` | **Authoritative** implementation spec for per-label stock images (roadmap #42). Rev 5. Category-agnostic. Supersedes the research doc where they conflict. | Implement stock images; resolve the `image_key` contract; decide hosting/caching/fallback/retry/versioning behaviour; compute and contain the label-name overlay; deliver consistent artwork across every app surface; write the app-side tests. | Verified key mismatch (only 21/28 matched); **26 decisions** including Firebase Hosting, static manifest, `image_key`/`labelSku`, 3-part lookup, app-owned cache, immutable printed name, shared `LabelArtwork`, demand-driven enrichment, explicit re-resolve boundaries, **content-cropped WebP q95 delivery**, deterministic glyph-table fitting with the app's bundled **Cormorant Garamond 600 SemiBold**, and **five bundled PNG fallbacks** (3 SKU + 2 category) with generated geometry; immutable hashed URLs; measured non-square delivery canvases; 240 pt fallback sizing contract; **59 behaviour contracts**; all 10 design answers; Phase 0-3 and mapped tests; 10 risks; rev-4 M2/M3 typography findings resolved. |
