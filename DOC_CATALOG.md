...
| `Docs\roadmap\Jar_Image_Strategy_Research.md` | Research + design for scaling placeholder jar art into a per-spice (and future per-quantity-level) catalog without bundling or per-spice app releases (roadmap #42, extends #10/#12). | Decide how to host/render a large stock-image matrix; plan per-spice/per-qty images; understand bundle/perf/storage tradeoffs. | Scale math (84→336 images); bundled-vs-remote recommendation (remote WebP + `expo-image` + small bundled fallback); deterministic URL + JSON manifest schema; stable `spice_key`/`quantityLevel` keys; app resolver + QR-service field changes; perf/memory (WebP, 480/720px variants, caching, lazy render); fallback chain; relationship to #10/#11/#12; phased plan (0–3); sourced citations. |
