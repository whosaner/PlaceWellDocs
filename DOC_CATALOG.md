...
| Deploy stock artwork to Firebase + Linode | `Docs\deployment\Stock_Image_Deployment.md` and `Docs\scripts\deploy_stock_images.ps1` |
...
| Understand system architecture and deep links | `Docs\architecture\Architecture_Diagrams.html` for ordinary images; `System_Overview.md` and `Visual_Call_Flows.md` for details |
...
| `Docs\architecture\Visual_Call_Flows.md` | Visual component map and Mermaid call-flow diagrams for the implemented PlaceWell system. | See exactly which systems participate in label production, in-app scanning, native-camera deep links, local save/recall, Order QR imports, and deployment. | Hosting boundaries; six sequence/flow diagrams; local-vs-Firestore data ownership; trust distinctions; runtime ownership and failure consequences. |
| `Docs\architecture\Firebase_Linode_SQL_Decision.md` | Current hosting inventory and database decision record. | Understand what runs on Linode versus Firebase, whether label metadata can move to SQL Server, and why Firestore remains the recommended current store. | Current/future service inventory; actual Firestore benefits used; Firestore vs SQL Server; managed PostgreSQL alternative; migration architecture and decision triggers. |
| `Docs\architecture\Architecture_Diagrams.html` | Browser-viewable static architecture gallery backed by PNG exports. | View all architecture diagrams without Mermaid support or a Markdown extension. | Ten high-resolution diagrams covering system boundaries, scan flows, local storage, order production, infrastructure ownership and database abstraction. |
| `Docs\context\PLACEWELL_CONTEXT.md` | Canonical product, business, architecture, release, and roadmap orientation for PlaceWell. | Give a team member or AI assistant one current overview before product or technical work. | Current product and release stage; ecosystem; mobile data ownership; QR trust; stock artwork; infrastructure; testing; closed decisions; priorities; canonical document map. |
...
| `Docs\deployment\Stock_Image_Deployment.md` | Paired Firebase Hosting + Linode stock-artwork runbook. | Deploy, verify, roll back, or provision permissions for immutable stock releases. | Dedicated/opt-in commands; release-ID layout; remote lock; staged verification; paired-target invariant; atomic `current`; SELinux/systemd permissions; failure and no-pruning policy. |
| `Docs\scripts\deploy.ps1` | One-shot PowerShell deploy script. Packages QR Service + PDF Generator + UI, uploads via scp, extracts on Linode, and restarts services. | Push local server-side code changes to Linode in a single command (does NOT deploy the mobile app); explicitly add stock deployment when needed. | Default UI/PDF/QR flow is unchanged; `-DeployStockImages` invokes the dedicated paired deployment afterward and propagates failures. Uses absolute paths — run from anywhere. |
| `Docs\scripts\deploy_stock_images.ps1` | Dedicated deterministic stock-artwork deployer. | Prepare and validate locally, deploy the identical release to Linode and Firebase, or run a safe local-only validation. | Calls the masters `prepare_release.ps1`; `-ValidateOnly`; verifies every WebP; stages and locks Linode; deploys Firebase from the masters package; verifies both; atomically switches `current`. |
...
| Document | What it covers | Look here if you want to... | Key contents |
|---|---|---|---|
| `Docs\roadmap\ROADMAP.md` | Unified product/project roadmap and backlog (single source of truth — the former separate todo list is folded in). | See launch status, shipped work, the full prioritized backlog in one table, and detailed feature notes. | Version 1.0 live on both stores; version 1.1 under review; condensed shipped summary; single **Master Priority List** table; post-launch privacy reminder; Next Up batch; feature details; related-docs index. |
| `Docs\roadmap\SDK57_Upgrade_And_Analytics_Plan.md` | Execution runbook to upgrade Expo SDK 54→57 and turn on Firebase Analytics (roadmap #9). | Plan the SDK upgrade + analytics activation, or follow the phased steps/risks. | Verified current state (app on SDK 54, RN 0.81; 55/56/57 shipped, 57 = latest; Firebase not installed, `GoogleService-Info.plist` present / `google-services.json` missing, wrapper wired); Phase 1 SDK 54→57 upgrade (`expo install --fix`, doctor, jest/Maestro/regression, step-through fallback); Phase 2 `@react-native-firebase` + iOS static frameworks + config files; Phase 3 privacy-declaration updates + build/submit; risks; definition of done. |
| `Docs\roadmap\Stock_Image_Implementation_Plan.md` | **Authoritative** implementation spec for per-label stock images (roadmap #42). Rev 7. Category-agnostic. Supersedes the research doc where they conflict. | Implement stock images; resolve the `image_key` contract; decide hosting/caching/fallback/retry/versioning behaviour; compute and contain the label-name overlay; deliver consistent artwork across every app surface; write the app-side tests. | **35 decisions** including paired exact-byte Firebase/Linode publication, immutable SHA-256 asset identity, verified atomic last-known-good writes, exact retry classes, global in-flight dedupe, four-transfer priority, retained active objects with native backup exclusion, usable-photo fallback, repurposed-label opt-out, `image_key`/`labelSku`, immutable printed name, shared `LabelArtwork`, content-cropped WebP q95, deterministic bundled-font fitting, and five bundled PNG fallbacks; **69 behaviour contracts**; all Phase-2 review findings resolved; Phase 0-3 and mapped tests. |
| `Docs\roadmap\Jar_Image_Strategy_Research.md` | Original research + design for scaling placeholder jar art into a per-spice (and future per-quantity-level) catalog (roadmap #42, extends #10/#12). **Partially superseded** — see the implementation plan above. | Understand the bundle-vs-remote tradeoff analysis, perf/memory reasoning, and sourced citations. | Scale math (now stale: 84→336); bundled-vs-remote recommendation (remote WebP + `expo-image` + small bundled fallback); deterministic URL + JSON manifest schema; stable `spice_key`/`quantityLevel` keys; app resolver + QR-service field changes; perf/memory (WebP, 480/720px variants, caching, lazy render); fallback chain; relationship to #10/#11/#12; phased plan (0–3); sourced citations. |
