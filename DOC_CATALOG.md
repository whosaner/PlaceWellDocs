...
| `Docs\roadmap\Stock_Jar_Image_Implementation_Plan.md` | **Authoritative** implementation spec for per-spice stock jar images (roadmap #42). Supersedes the research doc where they conflict. | Implement stock jar images; resolve the `spice_key` contract; decide hosting/caching/fallback/versioning behaviour. | Verified key mismatch (only 21/28 matched); 7 decisions (art IDs canonical, 26-spice catalog, Linode hosting, static manifest, runtime name overlay); reconciled CSV (3 renames, 2 additions, 4 removals); manifest + immutable content-hashed URL schema; Apache cache headers; answers to lookup/offline/download-once/label-text/photo add-remove/consistency/hosting/re-versioning; Phase 0 blocking server-side work (9 items incl. Firestore backfill + CI guard); Phases 1–3; 8 additional risks incl. `photoUri` cache-purge bug. |
