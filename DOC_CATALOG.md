...
| `Docs\roadmap\Stock_Jar_Image_Implementation_Plan.md` | **Authoritative** implementation spec for per-label stock jar images (roadmap #42). Rev 2. Supersedes the research doc where they conflict. | Implement stock jar images; resolve the `image_key` contract; decide hosting/caching/fallback/retry/versioning behaviour; write the app-side tests. | Verified key mismatch (only 21/28 matched); 14 decisions (art IDs canonical, 26-entry catalog, **Firebase Hosting**, static manifest, `image_key`/`labelSku` naming, 3-part `imageKey\|labelSku\|quantity` lookup, `defaultQuantity`, lossless WebP 720px, no bundled manifest, app-owned `documentDirectory` cache); reconciled CSV (3 renames, 2 additions, 4 removals); manifest schema + immutable content-hashed URLs; `firebase.json` cache headers; **31 numbered behaviour contracts (BC-01…BC-31)**; answers to all 10 original design questions; `spice_key`→`image_key` migration scope; masters-repo proposal (Git LFS); Phase 0 status; Phase 1 export; Phase 2 integration + **10 test suites mapped to contracts**; Phase 3 quantity levels; 10 risks incl. `photoUri` cache-purge bug and unverified iOS WebP decode. |
