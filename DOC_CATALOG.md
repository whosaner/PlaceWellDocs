...
| `Docs\roadmap\Stock_Image_Implementation_Plan.md` | **Authoritative** implementation spec for per-label stock images (roadmap #42). Rev 3. Category-agnostic. Supersedes the research doc where they conflict. | Implement stock images; resolve the `image_key` contract; decide hosting/caching/fallback/retry/versioning behaviour; compute the label-name overlay; write the app-side tests. | Verified key mismatch (only 21/28 matched); 17 decisions (art IDs canonical, 26-entry catalog, **Firebase Hosting**, static manifest, `image_key`/`labelSku` naming, 3-part `imageKey\|labelSku\|quantity` lookup, `defaultQuantity`, lossless WebP 720px, no bundled manifest, app-owned `documentDirectory` cache, **composite never cached, overlay uses the immutable printed name, persist `printedLabelName`**); reconciled CSV (3 renames, 2 additions, 4 removals); manifest schema + immutable content-hashed URLs; `firebase.json` cache headers; **label-name overlay geometry (contain-fit math, per-SKU normalised boxes, text fitting, name precedence)**; **40 numbered behaviour contracts (BC-01...BC-40)**; answers to all 10 original design questions; `spice_key`->`image_key` and jar/spice->agnostic rename scope; masters-repo proposal (Git LFS); Phase 0 status; Phase 1 export; Phase 2 integration + **12 test suites mapped to contracts**; Phase 3 quantity levels; 10 risks incl. `photoUri` cache-purge bug and unverified iOS WebP decode. |
