...
| `Docs\roadmap\Stock_Image_Implementation_Plan.md` | **Authoritative** implementation spec for per-label stock images (roadmap #42). Rev 4. Category-agnostic. Supersedes the research doc where they conflict. | Implement stock images; resolve the `image_key` contract; decide hosting/caching/fallback/retry/versioning behaviour; compute the label-name overlay; design the bundled fallback; write the app-side tests. | Verified key mismatch (only 21/28 matched); 23 decisions (art IDs canonical, **Firebase Hosting**, static manifest, `image_key`/`labelSku` naming, 3-part `imageKey\|labelSku\|quantity` lookup, `defaultQuantity`, lossless WebP 720px, no bundled manifest, app-owned `documentDirectory` cache, composite never cached, overlay uses the immutable printed name, persist `printedLabelName`, **new `LabelArtwork` component owning photo->stock->fallback, write-once demand-driven enrichment, no visible swap incl. the decode window, explicit re-resolve boundaries, geometry travels with the artwork, 3 SKU-accurate bundled fallback jars**); manifest schema + immutable content-hashed URLs; label-overlay geometry (contain-fit math, per-SKU boxes, name precedence); **48 behaviour contracts (BC-01...BC-48)**; answers to all 10 design questions; why capture-at-creation is insufficient (deep-link/transient-failure/legacy paths); rename scope; Phase 0-3 + **14 test suites mapped to contracts**; 10 risks; **12 unresolved MAJOR review findings (M1-M12) with the phase each must be decided by**. |
